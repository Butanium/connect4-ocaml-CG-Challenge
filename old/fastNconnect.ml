    ...
    let mcTree = ref ({base = {id=0; win=0.; visit=0.; turn=0; sons=[]; possAct=getActions 0 mctArray}; actNode=R}) in
    ...
    let expend node =
        match node.base.possAct with
            | x :: xs -> node.base.possAct <- xs;
                let possAct =  getActions node.base.turn mctArray in
                if List.mem (-2) possAct then prerr_endline @@ Printf.sprintf "added -2 at turn %d from father %d" node.base.turn node.base.id;
    ...
    let performActionNode arr = function
        | {actNode=N {action;_};base={turn; _}} -> performAction arr (turn mod 2) action
        | _ -> false
    in
