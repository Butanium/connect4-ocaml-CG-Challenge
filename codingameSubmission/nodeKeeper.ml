...
(* TODO add the perform check action for the tree *)
...
let performCheckAction arr player = function
    ...
    | j -> (let i = add arr player j in
           ...
           arr.(i).(j) <- -1; b)
in
let playouWinDebug = ref true in
let playout initTurn =
    ...
    let rec wh() = try (
        ...
        let r = auxW player actions in if r!= -66 then(
            ...
            performAction playoutArray player r; win()
        )
        else (
...
in
let rec canWin arr player = function
    | x :: xs ->  if performCheckAction arr player x then x else auxW player xs
    | _ -> -66
in
let expend node =
    match node.base.possAct with
        | x :: xs -> node.base.possAct <- xs;
            ...
            let r = canWin mctArray (1 - node.base.turn mod 2) possAct in
            if r != -66 
            if node.base.son = [] then
...
in
let mcTree = ref
    {base = {id=0; win=0. ; visit=0. ; turn=0; sons=[]; possAct=getActions 0 gameArray}; actNode=R}
in
...
while true do
    ...
    if turnindex > 0 then (
        ...
        mcTree := (try (let [x] = List.filter (fun x -> getActionFromNode x = oppAction) !mcTree.base.sons
        in {base=x.base; actNode=R}) with Match_failure _ -> newId := 1;
         {base = {id=0; win=0. ; visit=0. ; turn=turnindex; sons=[]; possAct=getActions 0 gameArray}; actNode=R});
